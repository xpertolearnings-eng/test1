...
class TradingJournalApp {
  ...
  handleAuthStateChange() {
    this.auth.onAuthStateChanged(async (user) => {
      this.showLoader(); // Show loader on auth state change
      if (user) {
      ...
      this.hideLoader(); // Hide loader after processing
    });
  }
    ...
    historyContainer.innerHTML = this.allNotes.map(note => {
      ...
      return `
        <div class="note-item clickable" onclick="app.showNoteDetails('${note.id}')">
            <div class="note-header"><h4 class="note-date">${new Date(note.date).toLocaleDateString('en-IN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</h4></div>
            <div class="note-content"><p>${notePreview.replace(/\n/g, '<br>')}</p></div>
        </div>
    `}).join('');
  }
  ...
  async saveRule() {
    ...
    const ruleData = { title, description, updatedAt: firebase.firestore.FieldValue.serverTimestamp() };
  ...
  /* ----------------------- ADD/EDIT TRADE FORM & MODALS ------------------------------ */

  renderAddTrade() {
    const now = new Date();
    const entryDateEl = document.querySelector('#addTradeForm input[name="entryDate"]');
    const exitDateEl = document.querySelector('#addTradeForm input[name="exitDate"]');
    ...
    this.renderRulebookChecklist('#rulebookChecklist');
  }

  renderRulebookChecklist(containerSelector) {
  ...
  async submitTrade() {
    ...
    const otherCharges = parseFloat(fd.get('otherCharges')) || 0;
    const trade = {
      symbol: fd.get('symbol').toUpperCase(), direction: fd.get('direction'), quantity: parseFloat(fd.get('quantity')),
      entryPrice: parseFloat(fd.get('entryPrice')), exitPrice: parseFloat(fd.get('exitPrice')),
      stopLoss: parseFloat(fd.get('stopLoss')) || null, targetPrice: parseFloat(fd.get('targetPrice')) || null,
      strategy: finalStrategy, exitReason: fd.get('exitReason') || 'N/A', confidenceLevel: parseInt(fd.get('confidenceLevel')),
      entryDate: fd.get('entryDate'), exitDate: fd.get('exitDate'), preEmotion: fd.get('preEmotion') || '', postEmotion: fd.get('postEmotion') || '',
      notes: fd.get('notes') || '', sleepQuality: parseInt(fd.get('sleepQuality')) || 5, physicalCondition: parseInt(fd.get('physicalCondition')) || 5,
      marketSentiment: fd.get('marketSentiment') || '', newsAwareness: fd.get('newsAwareness') || '', marketEnvironment: fd.get('marketEnvironment') || '',
      fomoLevel: parseInt(fd.get('fomoLevel')) || 1, preStress: parseInt(fd.get('preStress')) || 1,
      multiTimeframes: this.getCheckboxValues(form, 'multiTimeframes'), volumeAnalysis: fd.get('volumeAnalysis') || '',
      technicalConfluence: this.getCheckboxValues(form, 'technicalConfluence'), marketSession: fd.get('marketSession') || '',
      tradeCatalyst: fd.get('tradeCatalyst') || '', waitedForSetup: this.getRadioValue(form, 'waitedForSetup'),
      positionComfort: parseInt(fd.get('positionComfort')) || 5, planDeviation: fd.get('planDeviation') || '',
      stressDuring: parseInt(fd.get('stressDuring')) || 1, primaryExitReason: fd.get('primaryExitReason') || '',
      exitEmotion: fd.get('exitEmotion') || '', wouldTakeAgain: this.getRadioValue(form, 'wouldTakeAgain'),
      lesson: fd.get('lesson') || '', volatilityToday: fd.get('volatilityToday') || '', sectorPerformance: fd.get('sectorPerformance') || '',
      economicEvents: this.getCheckboxValues(form, 'economicEvents'), personalDistractions: this.getCheckboxValues(form, 'personalDistractions'),
      followedRules: this.getCheckboxValues(form, 'followedRules'),
      commission, otherCharges, createdAt: firebase.firestore.FieldValue.serverTimestamp()
    };
  ...
  showTradeDetails(id) {
    ...
    if (t.followedRules && t.followedRules.length > 0) {
      followedRulesHtml = `
            <div style="margin-top:16px;"><strong>Followed Rules:</strong><ul style="padding-left: 20px; margin-top: 8px;">
            ${t.followedRules.map(rule => `<li>${rule}</li>`).join('')}</ul></div>`;
    }
  ...
  showEditTradeModal() {
    this.hideTradeModal();
    const trade = this.allTrades.find(t => t.id === this.currentEditingTradeId);
    if (!trade) {
    ...
    const allStrategies = ["Breakout + VWAP", "Support Resistance", "Trendline Bounce", "Breakout Failure", "Scalping", "Swing Trading", "Momentum Trading", "Position Trading", "Mean Reversion", "Pairs Trading", "Arbitrage", "News-Based Trading", "Algorithmic Trading", "Carry Trade", "Elliott Wave Theory", "Gann Theory", "Other"];
    
    const rulesChecklistHtml = this.allRules.length > 0 
        ? this.allRules.map(rule => `
            <label>
                <input type="checkbox" name="followedRules" value="${rule.title}" ${trade.followedRules?.includes(rule.title) ? 'checked' : ''}>
                ${rule.title}
            </label>`).join('')
        : '<p class="empty-state-sm">No rules defined.</p>';

    modalBody.innerHTML = `
        <form id="editTradeForm" novalidate>
            <div class="form-grid">
                <div class="form-group"><label class="form-label">Symbol</label><input type="text" name="symbol" class="form-control" value="${trade.symbol}" required></div>
                <div class="form-group"><label class="form-label">Direction</label><select name="direction" class="form-control" required><option value="Long" ${trade.direction === 'Long' ? 'selected' : ''}>Long</option><option value="Short" ${trade.direction === 'Short' ? 'selected' : ''}>Short</option></select></div>
                <div class="form-group"><label class="form-label">Quantity</label><input type="number" name="quantity" class="form-control" value="${trade.quantity}" required></div>
                <div class="form-group"><label class="form-label">Entry Price</label><input type="number" name="entryPrice" class="form-control" value="${trade.entryPrice}" required step="any"></div>
                <div class="form-group"><label class="form-label">Exit Price</label><input type="number" name="exitPrice" class="form-control" value="${trade.exitPrice}" required step="any"></div>
                <div class="form-group"><label class="form-label">Commission</label><input type="number" name="commission" class="form-control" value="${trade.commission || 0}" step="any"></div>
                <div class="form-group"><label class="form-label">Other Charges</label><input type="number" name="otherCharges" class="form-control" value="${trade.otherCharges || 0}" step="any"></div>
                <div class="form-group"><label class="form-label">Strategy</label><select name="strategy" class="form-control">${allStrategies.map(s => `<option value="${s}" ${trade.strategy === s ? 'selected' : ''}>${s}</option>`).join('')}</select></div>
                <div class="form-group"><label class="form-label">Stress (1-10)</label><input type="range" name="preStress" class="range-input" min="1" max="10" value="${trade.preStress || 1}"><div class="range-value">${trade.preStress || 1}</div></div>
                <div class="form-group"><label class="form-label">FOMO (1-10)</label><input type="range" name="fomoLevel" class="range-input" min="1" max="10" value="${trade.fomoLevel || 1}"><div class="range-value">${trade.fomoLevel || 1}</div></div>
            </div>
            <details class="collapse-card" open>
                <summary>Rulebook Check</summary>
                <div class="collapse-content">
                    <div class="checkbox-group">${rulesChecklistHtml}</div>
                </div>
            </details>
    ...
    modalBody.querySelectorAll('.range-input').forEach(slider => {
      const display = slider.parentElement.querySelector('.range-value');
      if (display) slider.addEventListener('input', () => (display.textContent = slider.value));
    });
  ...
  async saveTradeChanges() {
    ...
    const updatedData = {
        ...
        otherCharges: parseFloat(fd.get('otherCharges')) || 0,
        strategy: fd.get('strategy'),
        preStress: parseInt(fd.get('preStress')),
        fomoLevel: parseInt(fd.get('fomoLevel')),
        followedRules: this.getCheckboxValues(form, 'followedRules'),
        updatedAt: firebase.firestore.FieldValue.serverTimestamp()
    };
    
    updatedData.grossPL = updatedData.direction === 'Long' 
        ? (updatedData.exitPrice - updatedData.entryPrice) * updatedData.quantity 
        : (updatedData.entryPrice - updatedData.exitPrice) * updatedData.quantity;
    updatedData.netPL = updatedData.grossPL - updatedData.commission - updatedData.otherCharges;
    
    const originalTrade = this.allTrades.find(t => t.id === this.currentEditingTradeId);
    if (originalTrade && originalTrade.stopLoss && originalTrade.targetPrice) {
        const risk = Math.abs(updatedData.entryPrice - originalTrade.stopLoss);
        const reward = Math.abs(originalTrade.targetPrice - updatedData.entryPrice);
        updatedData.riskRewardRatio = risk > 0 ? reward / risk : 0;
    }
  ...
  drawDashboardPLChart() {
    ...
    if (!ctx) return;
    if (this.charts.dashboardPl) this.charts.dashboardPl.destroy();
    ...
    this.charts.dashboardPl = new Chart(ctx, {
      ...
      data: {
        labels: labels,
        datasets: [{
          label: 'Cumulative P&L', data: data, borderColor: 'rgba(31, 184, 205, 0.8)', backgroundColor: 'rgba(31, 184, 205, 0.1)',
      ...
      options: {
        ...
        scales: {
          y: { beginAtZero: false, ticks: { font: { size: 10 }, callback: (value) => this.formatCurrency(value).replace('.00', '') } },
          x: { display: false }
        }
      }
    });
  ...
  renderAISuggestions() {
    if (this.trades.length < 3) {
      document.getElementById('smartInsight').textContent = "Add at least 3 trades for AI suggestions.";
    ...
    document.getElementById('smartInsight').textContent = s.totalPL >= 0 ?
      ...
      `You're net negative ${this.formatCurrency(s.totalPL)}. Focus on risk management.`;
  ...
  drawConfidenceChart() {
    ...
    this.charts.conf = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{ label: 'Confidence', data: sorted.map(c => c.level), borderColor: '#1FB8CD', tension: 0.3 }]
      },
      options: {
        responsive: true, maintainAspectRatio: false,
        plugins: { legend: { display: false } },
  ...
  bestStrategy() {
    ...
    const map = {};
    this.trades.forEach(t => {
      if (t.strategy && t.strategy !== 'N/A') map[t.strategy] = (map[t.strategy] || 0) + t.netPL;
    });
  ...
  generateAIFeedback() {
    ...
    let feedback = '';
    if (stats.winRate < 50) feedback += `<div class="suggestion-item suggestion-warning">Your win rate is ${stats.winRate}%. Review your entry criteria.</div>`;
    else feedback += `<div class="suggestion-item suggestion-good">Your win rate of ${stats.winRate}% is solid. Keep refining!</div>`;
    const avgRR = parseFloat(stats.avgRR.split(':')[1]);
    if (avgRR < 1.5) feedback += `<div class="suggestion-item suggestion-warning">Your average R:R is low (${stats.avgRR}). Aim for higher potential reward.</div>`;
    return feedback;
  }

  analyzeTradingEdge() {
    const best = this.bestStrategy();
    const worstStrategies = Object.entries(this.trades.reduce((acc, t) => { if (t.strategy) acc[t.strategy] = (acc[t.strategy] || 0) + t.netPL; return acc; }, {})).sort((a, b) => a[1] - b[1]);
    ...
    if (worstStrategies.length > 0 && worstStrategies[0][1] < 0) result += `<div class="suggestion-item suggestion-warning">Your least profitable strategy is <strong>${worstStrategies[0][0]}</strong>. Consider re-evaluating it.</div>`;
    return result;
  }
  ...
  findRepeatLosingTrades() {
    ...
    const patternCount = {};
    losingTrades.forEach(t => { if (t.strategy) { const p = `${t.symbol} on ${t.strategy}`; patternCount[p] = (patternCount[p] || 0) + 1; } });
    const recurringLosses = Object.entries(patternCount).filter(([, count]) => count > 1).sort((a, b) => b[1] - a[1]);
    if (recurringLosses.length > 0) return `<div class="suggestion-item suggestion-warning">You have recurring losses with: <strong>${recurringLosses[0][0]}</strong> (${recurringLosses[0][1]} times). Analyze these trades.</div>`;
    return '<div class="suggestion-item suggestion-info">No significant recurring losing patterns detected.</div>';
  }

  analyzeEntries() {
    const fomoTrades = this.trades.filter(t => t.fomoLevel > 5 && t.netPL < 0);
    if (fomoTrades.length > 2) return `<div class="suggestion-item suggestion-warning">You've made ${fomoTrades.length} losing trades with high FOMO. Avoid chasing.</div>`;
    const earlyEntries = this.trades.filter(t => t.waitedForSetup === 'No, entered early' && t.netPL < 0);
    if (earlyEntries.length > 2) return `<div class="suggestion-item suggestion-warning">You have ${earlyEntries.length} losing trades where you entered early. Practice patience.</div>`;
    return '<div class="suggestion-item suggestion-good">Your entries appear disciplined.</div>';
  }

  analyzeEmotionalBias() {
    const frustrationExits = this.trades.filter(t => t.exitEmotion === 'Frustrated' && t.netPL < 0);
    if (frustrationExits.length > 2) return `<div class="suggestion-item suggestion-warning">You've exited ${frustrationExits.length} losing trades feeling frustrated. Take a break after a loss.</div>`;
    const fearExits = this.trades.filter(t => t.primaryExitReason === 'Fear-based exit');
    if (fearExits.length > 2) {
      const profitLeft = fearExits.reduce((sum, t) => { if (t.netPL > 0 && t.targetPrice > t.exitPrice) return sum + (t.targetPrice - t.exitPrice) * t.quantity; return sum; }, 0);
      return `<div class="suggestion-item suggestion-warning">You've exited ${fearExits.length} trades due to fear, potentially leaving ${this.formatCurrency(profitLeft)} on the table.</div>`;
    }
    return '<div class="suggestion-item suggestion-info">Your emotional responses seem balanced.</div>';
  }
  ...
  calculateSetupQuality() {
    ...
    return `<div class="suggestion-item suggestion-info">For trades with 3+ confluence factors, your win rate is <strong>${stats.winRate}%</strong> with a P&L of <strong>${this.formatCurrency(stats.totalPL)}</strong>. Prioritize these setups.</div>`;
  }

  analyzeTimeBasedConfidence() {
    const morningTrades = this.trades.filter(t => t.marketSession === 'Market Open (9:30-10:30)');
    if (morningTrades.length < 3) return '<div class="empty-state">Not enough trades during market open.</div>';
    const stats = this.calculateStatsForTrades(morningTrades);
    if (stats.totalPL > 0) return `<div class="suggestion-item suggestion-good">You perform well during the market open, with a P&L of <strong>${this.formatCurrency(stats.totalPL)}</strong>.</div>`;
    return `<div class="suggestion-item suggestion-warning">You struggle during the market open, with a P&L of <strong>${this.formatCurrency(stats.totalPL)}</strong>. Consider trading smaller.</div>`;
  }

  calculateStatsForTrades(trades) {
    if (trades.length === 0) return { totalPL: 0, winRate: 0, totalTrades: 0, bestTrade: 0, worstTrade: 0 };
  ...
  changeCalendarMonth(offset) {
    ...
    if (document.getElementById('dashboard').classList.contains('active')) this.buildDashboardCalendar();
    if (document.getElementById('reports').classList.contains('active')) this.buildCalendar();
  }
  ...
  buildCalendar() {
    ...
    for (let d = 1; d <= monthEnd.getDate(); d++) {
      ...
      if (tradesOnDay.length > 0) {
        ...
        if (pl > 1000) cls = 'profit-high'; else if (pl > 0) cls = 'profit-low'; else if (pl < -1000) cls = 'loss-high'; else if (pl < 0) cls = 'loss-low'; else cls = 'profit-low';
  ...
  showCalendarTooltip(event, trades, dateKey) {
    ...
    trades.forEach(trade => {
      content += `<div class="calendar-tooltip-trade"><span class="symbol">${trade.symbol}</span><span class="pl ${trade.netPL >= 0 ? 'positive' : 'negative'}">${this.formatCurrency(trade.netPL)}</span></div>`;
    });
  ...
  generateWeeklyReport() {
    ...
    return `<div class="report-item"><span>Trades:</span><span>${stats.totalTrades}</span></div>
  ...
  generateMonthlyReport() {
    ...
    const monthlyTrades = this.trades.filter(t => { const d = new Date(t.entryDate); return d.getMonth() === thisMonth && d.getFullYear() === thisYear; });
  ...
  generateStrategyReport() {
    const byStrategy = {};
    this.trades.forEach(t => { if (t.strategy) { if (!byStrategy[t.strategy]) byStrategy[t.strategy] = []; byStrategy[t.strategy].push(t); } });
    ...
    for (const strategy in byStrategy) {
      ...
      table += `<tr><td>${strategy}</td><td>${stats.totalTrades}</td><td>${stats.winRate}%</td><td class="${stats.totalPL >= 0 ? 'positive' : 'negative'}">${this.formatCurrency(stats.totalPL)}</td></tr>`;
    }
  ...
  generateEmotionalReport() {
    const preEmotions = {};
    this.trades.forEach(t => { if (t.preEmotion) { if (!preEmotions[t.preEmotion]) preEmotions[t.preEmotion] = { total: 0, pl: 0 }; preEmotions[t.preEmotion].total++; preEmotions[t.preEmotion].pl += t.netPL; } });
    if (Object.keys(preEmotions).length === 0) return '<div class="empty-state">No emotional data recorded.</div>';
    let mostProfitable = { emotion: 'N/A', avg: -Infinity }; let leastProfitable = { emotion: 'N/A', avg: Infinity };
    ...
    return `<div class="report-item"><span>Most Profitable Emotion:</span><span>${mostProfitable.emotion} (${this.formatCurrency(mostProfitable.avg)}/trade)</span></div>
  ...
  generateRulebookReport() {
    if (this.allRules.length === 0) return '<div class="empty-state">No rules defined.</div>';
    if (this.allTrades.length === 0) return '<div class="empty-state">No trades recorded.</div>';
    const ruleStats = {};
    this.allRules.forEach(rule => { ruleStats[rule.title] = { followed: 0 }; });
    this.allTrades.forEach(trade => { if (trade.followedRules) trade.followedRules.forEach(title => { if (ruleStats[title]) ruleStats[title].followed++; }); });
    ...
    for (const title in ruleStats) {
      ...
      table += `<tr><td>${title}</td><td>${followed} / ${totalTrades}</td><td><div class="progress-bar"><div class="progress-bar-fill" style="width: ${adherence}%;">${adherence}%</div></div></td></tr>`;
    }
    return table + '</tbody></table>';
  }
